<?php
$string['pluginname'] = 'Courseboard';
$string['userpreferences'] = 'Einstellungen';
$string["modulename"] = "Courseboard";
$string["modulenameplural"] = "Courseboard";
$string['modulename_help'] = "Dieses Plugin ermöglicht den Studenten schnell und einfach dem Kurs oder dem Lehrenden Rückmeldung,in Form von einem kleinen Text, zu geben.
Ein Eintrag kann man ohne das sein Name mitgesendet wird schreiben, das heißt die Studenten haben eine höhere Motivation mitzumachen.
Diese Einträge werden auf eine 'Wand' dargestellt, so kann man sich schnell und einfach einen Überblick über die Situation verschaffen. 
Mit der Option ein Eintrag, zu kommentieren kann man auf ein Eintrag reagieren und zum Beispiel eine Frage beantworten."; 
$string["newmodulename"] = "Neuer Modulname";
$string["newmoduleintro"] = "Beschreibung";
$string['pluginadministration'] = "Pluginadministration";
$string["nameinputdescription"] = 'Wenn nicht dein Name ausgewählt ist, wird Anonymus als dein Name abgesendet.';
$string["oldsortdescription"] = "älteste";
$string["newsortdescription"] = "neueste";
$string["ratingdescending"] = "rating absteigend";
$string["ratingascending"] = "rating aufsteigend";
$string["feedbackwall"] = "Courseboard";
$string["noComments"] = "Es gibt noch kein Kommentar zu diesem Eintrag";
$string["noFeedbacks"] ="Es gibt noch kein Eintrag";
$string["send"] = "Senden";
$string["name"] = "Name";
$string["rating"] = "Anzahl der Bewertungen";
$string["rate"] = "bewerten";
$string["showComments"] = "Kommentare anzeigen";
$string["hideComments"] = "Kommentare ausblenden";
$string["writeaFeedback"] = "Schreibe ein Eintrag...";
$string["writeaComment"] = "Schreibe ein Kommentar...";
$string["loadingpleasewait"] = "Laden, bitte warten...";
$string["emptyFeedbackinput"] = "Schreibe etwas um ein Eintrag absenden zu können!";
$string["emptyCommentinput"] = "Schreibe etwas um ein Kommentar absenden zu können!";
$string["alreadyrated"] = "Du hast diesen Eintrag schon bewertet!";
$string["rateFeedback"] = "Bewertet diese Eintrag...";
$string["rateoneStar"] = "(1) Eintrag interessiert mich nicht";
$string["ratetwoStars"] = "(2) Eintrag passt nur teilweise";
$string["ratethreeStars"] = "(3) Eintrag betrifft mich";
$string["ratefourStars"] = "(4) Eintrag ist gut";
$string["ratefiveStars"] = "(5) Eintrag könnte von mir sein";
$string["amountdescending"] ="Anzahl der Bewertungen absteigend";
$string["amountascending"] ="Anzahl der Bewertungen aufsteigend";
$string["anonymous"] = "Anonymus";
$string["sortfor"] = "Sortieren nach";
